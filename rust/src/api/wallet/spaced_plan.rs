...
use crate::core::wallet_persistence::labels::{
    address_has_explicit_label, get_address_label_with_source, get_coin_label_with_source,
    set_address_label,
};
...
use crate::core::wallet_persistence::tx_plan_storage::{
    ...
    list_unsigned_tx_ids_for_plan, reconcile_completed_tx_plans, set_tx_plan_status,
    set_unsigned_tx_plan_id, NewTxPlan, TxPlanKind, TxPlanRow, TxPlanStatus,
};
...
impl APIWallet {
    ...
    pub fn plan_spaced_txs(&self, params: APISpacedPlanParams) -> Result<APISpacedPlanSummary> {
        ...
        //    re-entry fails fast.
        {
            ...
            // Heal any orphan SIGNED plan whose children were all
            // auto-broadcast before the explicit DONE transition shipped.
            reconcile_completed_tx_plans(&core.conn)?;
...
/// `Migrate` plans).
/// Resolve the on-chain address that holds a given source UTXO. Returns
/// `None` when the outpoint is not in the wallet's graph or its script
/// belongs to a foreign keychain (shouldn't happen for spaced plans, but
/// keeps the call infallible).
fn source_address(wallet: &bdk_wallet::Wallet, txid_str: &str, vout: u32) -> Option<String> {
    use std::str::FromStr;
    let txid = bdk_wallet::bitcoin::Txid::from_str(txid_str).ok()?;
    let outpoint = bdk_wallet::bitcoin::OutPoint::new(txid, vout);
    let txout = wallet.tx_graph().get_txout(outpoint)?;
    let (keychain, index) = wallet
        .spk_index()
        .index_of_spk(txout.script_pubkey.clone())?;
    Some(wallet.peek_address(*keychain, *index).address.to_string())
}

/// Resolve the label that the planner should propagate forward for a given
/// source UTXO. Reads the coin label first; if missing (or generated by a
/// previous spaced plan), falls back to the address label under the same
/// rule. Returns `None` if neither carries a usable label, so the caller
/// can drop a generated fallback in.
fn resolve_inheritable_label(
    conn: &rusqlite::Connection,
    outpoint_key: &str,
    src_address: Option<&str>,
) -> Result<Option<String>> {
    fn usable(row: Option<(String, bool, Option<String>)>) -> Option<String> {
        let (label, _is_auto, source) = row?;
        if label.is_empty() {
            return None;
        }
        let src = source.as_deref().unwrap_or("");
        if src.starts_with("spaced_plan:") {
            return None;
        }
        Some(label)
    }

    if let Some(label) = usable(get_coin_label_with_source(conn, outpoint_key)?) {
        return Ok(Some(label));
    }
    if let Some(addr) = src_address {
        if let Some(label) = usable(get_address_label_with_source(conn, addr)?) {
            return Ok(Some(label));
        }
    }
    Ok(None)
...
fn propagate_source_label(
    ...
    // The "inheritable" label is the source UTXO's effective label
    // (coin first, falling back to the source address), or a generated
    // fallback when none is available. It flows to the destination
    // addresses in both modes — directly here for `Refresh`, and via the
    // cubit (`set_spaced_plan_address_labels`) for `Migrate`. This is also
    // the value returned to the FFI summary so the Flutter side can seed
    // the destination wallet without re-reading the source DB.
    //
    // Labels stamped by a previous spaced plan (`source_entity` starts
    // with `spaced_plan:`) are ignored — otherwise a second refresh of a
    // child UTXO would copy the stale auto-generated date forward
    // indefinitely. The check uses `source_entity` rather than `is_auto`
    // so legitimate user-propagated auto-labels (from `tx:` / `addr:`
    // sources) are still inherited.
    let inheritable = resolve_inheritable_label(conn, &outpoint_key, src_address)?
        .unwrap_or_else(|| default_plan_label(kind, src_txid, src_vout));
    ...
    fn build_intent_psbts(
        ...
        for intent in intents.into_iter() {
            ...
            let label = {
                ...
                let src_address = source_address(&core.wallet, &utxo_txid, utxo_vout);
