#!/usr/bin/env python3
"""
Regression test 35: HW signs a 1-of-2 sh(wsh(sortedmulti(...))) PSBT (BIP48
nested SegWit multisig) and the node accepts the broadcast.

Distinct from reg34 because:
  - Outer wrapper is sh(wsh(...)) so parse_bip48_sortedmulti returns
    ScriptType::P2wshP2sh — different firmware path than P2wsh.
  - HW key path uses the BIP48 script_type=1 sub-account (m/48'/1'/0'/1').
...
WALLET_NAME = "Reg35-HW-Sign-ShWsh-1of2"
HW_DERIVATION_PATH = "m/48'/1'/0'/1'"   # BIP48 script_type=1 → P2SH-P2WSH
PSBT_LABEL = "reg35-hw-shwsh-selfpay"

COSIGNER_MFP = "ff81be5d"
COSIGNER_PATH = "48'/1'/0'/1'"
# Reuses the same watch-only xpub as reg34 — the BIP48 sub-account
# script_type only affects the HW key path. The cosigner xpub is treated as
# opaque key material in the descriptor.
COSIGNER_XPUB = (
...
async def _create_wallet(d: UIDriver):
    print("\n  [phase 1] create 1-of-2 sh(wsh(sortedmulti)) (HW + watch-only)")
    ...
    # Nested SegWit (P2SH-P2WSH). Per project_ui_naming, the chip label is
    # "Nested" (not translated).
    ...
    hw_mfp = await connect_hw_in_open_sheet(d, op_label="export xpub for sh-wsh multisig")
...
async def _register_on_hw(d: UIDriver):
    ...
    await connect_hw_in_open_sheet(d, op_label="sh-wsh multisig register")
    ...
    if initial == "absent":
        ...
        await wait_for_user("Confirm the wallet registration on the BitBox.")
...
async def _sign_with_hw(d: UIDriver):
    ...
    await connect_hw_in_open_sheet(d, op_label="sign sh-wsh multisig PSBT")
...
async def test_hw_sign_shwsh_multisig(d: UIDriver):
    print(f"\n--- {WALLET_NAME} ---")
    await set_active_network_signet(d)
    await _create_wallet(d)
    await _register_on_hw(d)
    await verify_first_receive_address_on_hw(d, op_label="verify sh-wsh multisig address")
...
if __name__ == "__main__":
    asyncio.run(run_regression(test_hw_sign_shwsh_multisig, "reg35"))
