#!/usr/bin/env python3
"""
Regression test 37: HW signs a 1-of-2 taproot multisig descriptor with NO
script tree depth — pure tr(NUMS, multi_a(1, A, B)) — and the node accepts
the broadcast.

Distinct from reg31: there is a single tap leaf and the HW key is in only
one place. This is the "clean" Policy variant on taproot, contrasting with
reg31 where the HW key appears in five different leaves.
...
PROJECT_NAME = "Reg37-HW-TrMulti"
WALLET_NAME = "Reg37-HW-TrMulti-1of2"
PSBT_LABEL = "reg37-hw-tr-multi-selfpay"

# Internal key is the BIP341 NUMS-style "unspendable" xpub used in reg31.
# tap leaf is multi_a(1, HW, watch-only) — taproot variant of reg34.
DESCRIPTOR = (
    "tr("
    ...
    "[ff81be5d/48'/1'/0'/2']"
    "tpubDDxjvuVfYHF4KcVyd5wkNS6pKJvg1x6CUtCRL3nRX2MDHKcja6M7YB7FYFYDkXzx8fL7k9bYi8XDpfPetqvd6ER2VYt1WsQSHYnhhT2EX7K/<0;1>/*"
    ")"
    ")"
)
...
async def _register_on_hw(d: UIDriver):
    print("\n  [phase 3] register taproot multisig policy on HW (idempotent)")
    ...
    await connect_hw_in_open_sheet(d, op_label="tr multisig register")
    ...
    if initial == "absent":
        ...
        await wait_for_user("Confirm the taproot multisig policy on the BitBox.")
...
async def _build_psbt(d: UIDriver):
    print("\n  [phase 5] build self-pay MAX PSBT")
...
async def _sign_with_hw(d: UIDriver):
    print("\n  [phase 6] sign with HW")
    await click_label(d, "Sign with hardware wallet", delay=0.5)
    await connect_hw_in_open_sheet(d, op_label="sign tr multisig PSBT")
...
async def test_hw_sign_tr_multisig(d: UIDriver):
    print(f"\n--- {WALLET_NAME} ---")
    await set_active_network_signet(d)
    await _import_and_create_wallet(d)
    await _register_on_hw(d)
    await verify_first_receive_address_on_hw(d, op_label="verify tr multisig address")
...
if __name__ == "__main__":
    asyncio.run(run_regression(test_hw_sign_tr_multisig, "reg37"))
