#!/usr/bin/env python3
"""
Regression test 32: Hardware wallet P2TR (BIP86) singlesig — verify, sign,
broadcast.

Exercises the SimpleType=P2TR (Schnorr) firmware path on the BB02. Distinct
from reg30 (BIP84/wpkh): the dispatch in btc_sign_psbt:325-330 detects
single-key tr by `is_native_tr` and must keep `device.policy = None`.

Flow:
  1. Set Signet.
  2. Guided create tr() wallet from HW xpub at m/86'/1'/0'.
  3. Sync wallet. If empty → FAIL with receive address (operator funds + reruns).
  4. Verify-address on device for receive_address_0 (P2TR Show on device).
  5. Send → self-pay MAX → Create PSBT.
  6. Sign with HW.
  7. Broadcast → assert "Broadcasted".
  8. Cleanup: delete wallet.

Skips ONLY if no HW device is detected (per `connect_hw_in_open_sheet`).
...
WALLET_NAME = "Reg32-HW-Sign-P2TR"
DERIVATION_PATH = "m/86'/1'/0'"
PSBT_LABEL = "reg32-hw-tr-selfpay"


async def _create_wallet_with_hw_xpub(d: UIDriver):
    print("\n  [phase 1] create P2TR singlesig from HW xpub")
    ...
    # Pick Taproot type. Default for singlesig is usually SegWit (wpkh); switch.
    ...
    mfp = await connect_hw_in_open_sheet(d, op_label="export xpub (P2TR)")
...
async def _verify_address_on_device(d: UIDriver):
    await verify_first_receive_address_on_hw(d, op_label="verify P2TR address")
...
async def _sign_with_hw(d: UIDriver):
    ...
    await connect_hw_in_open_sheet(d, op_label="sign P2TR PSBT")
...
async def test_hw_sign_p2tr(d: UIDriver):
    print(f"\n--- {WALLET_NAME} ---")
...
if __name__ == "__main__":
    asyncio.run(run_regression(test_hw_sign_p2tr, "reg32"))
