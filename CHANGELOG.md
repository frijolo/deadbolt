...
### Improvements
- **Hot badge on spend paths** — The Descriptor tab's spend-path list now shows the same "HOT" badge as the Keys tab when a path is fully satisfied by private key material stored in the app.
...
### Fixes
- **False on-chain backup detection** — Backup recovery now requires a candidate reveal transaction to actually carry a Deadbolt inscription envelope, rejecting an unrelated transaction that merely spends the commit's change output. Candidate commits are also scanned most-recent-first deterministically, and an empty wallet-address hash can no longer match a backup as a false positive across wallets that share anchors.
- **RBF minimum fee too high** — The minimum replacement fee shown when bumping a transaction now uses Bitcoin Core's actual `incrementalrelayfee` (0.1 sat/vB) instead of 1 sat/vB, so the suggested fee no longer overshoots BIP-125 Rule 4.
...
### Improvements
- **Lower battery use while minimised** — On Android, Electrum subscriptions are torn down ~2 minutes after the app is backgrounded and re-established on resume with a catch-up sync, instead of holding one TCP+TLS socket per wallet open indefinitely.
- **Spacing terminology in TX planning** — Renamed "delay" to "spacing" across the TX-planning idle view (labels, helper text, validation messages) and added a live ETA preview that shows the expected `min–max` blocks and wall-clock window for the planned batch.
...
- **Publish backup as top-level export action** — The wallet Export menu surfaces Publish Backup as its own entry next to Descriptor and Wallet, instead of hiding it inside the descriptor export sheet.
- **Coin selector searches by label** — Typing in the Select Coins search bar now matches the coin label in addition to address and amount.
- **Consistent error messages** — Wallet list, project list, restore, sweep, and biometric lock errors are now formatted through `formatRustError` so users see a clean message instead of the raw Dart/Rust exception.
